...
class Auth extends Component {
    ...
    authHandler = (e) => {
        e.preventDefault();
        this.props.sendAuth(this.state.authFormData.email.value, this.state.authFormData.password.value, this.state.isSignup);
    }
    ...
    inputChangeHandler = (event, id) => {
        ...
        const clonedAuthFormData = updateObject(this.state.authFormData, {
            [id]: updateObject(this.state.authFormData[id], {
                value: event.target.value,
                valid: checkValidation(event.target.value, this.state.authFormData[id].validation),
                touched: true
            })
    ...
    render() {
        ...
        for(let key in this.state.authFormData) {
            formElementsArray.push({
            id: [key],
            config: this.state.authFormData[key]
            });
        }
        ...
        if(this.props.errorState) {
            errorMsg = <p className={classes.AuthErrorMsg}>{this.props.errorState.message}</p>;
        }
        ...
        return (
            <div className={classes.Auth}>
                ...
                {form}
                <Button btnType= "Danger" clicked={this.authSwitchModeHandler}>Switch to {this.state.isSignup ? "Signin" : "Signup"}</Button>
                {errorMsg}
            </div>
        )
    }
